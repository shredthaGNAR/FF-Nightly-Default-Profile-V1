...
UC.privateTab = {
  ...
  toggleMask: function (win) {
    let {gBrowser} = win;
    let privateIndicator = win.document.querySelector("#TabsToolbar > .private-browsing-indicator-with-label");
    ...
    this.STYLE = {
      url: Services.io.newURI('data:text/css;charset=UTF-8,' + encodeURIComponent(`
        @-moz-document url('${_uc.BROWSERCHROME}'), url-prefix('chrome://browser/content/places/') {
          #TabsToolbar > .private-browsing-indicator-with-label[enabled="true"] {
            display: inherit !important;
          }

          #main-window:not([privatebrowsingmode]) #TabsToolbar > .private-browsing-indicator-with-label label {
            display: none;
          }
  ...
  destroy: function () {
    ...
    _uc.windows((doc, win) => {
      ...
      doc.querySelector("#TabsToolbar > .private-browsing-indicator-with-label").removeAttribute('enabled');
    }, false);
